...
) const
{
    ...
    //Kinetic viscosity
    const volScalarField muk = (48/
	    (5*sqrtPi)*alpha-2./5*(1+e)*(1-3*e)*alpha*g0)/
	    ((1-0.25*pow((1-e), 2)-5./24*(1-pow(e, 2)))*g0);
    //Contact viscosity
    const volScalarField muc = muk*(4./5*(1+e)*alpha*g0);
    //Bulk viscosity
    const volScalarField mub = 384./(25*pi)*(1+e)*pow(alpha, 2)*g0;

    //Total viscosity
    const volScalarField muTot = muk + muc + mub;
    return rhoa*da*sqrt(Theta)*5*sqrtPi/96*muTot;
}
