...
gs.update(left=0.1, right=0.95, top=0.95, bottom=0.2, wspace=0.125, hspace=0.25)
#
# Figure size
#
figwidth = 15
...
[
    ...
    duexdz,
    nuex,
] = analyticBagnold.analyticBagnold(
    nx, H, g, d, rho_p, rho_f, phi0, I0, Bphi, mus, mu2, beta
)
print(
    "max(uex)=" + str(np.max(uex)) + " m/s" + "max(phi)=",
    np.max(alphaex),
    "max(nuex)=",
    np.max(nuex),
)
...
(l22,) = ax2.plot(
    (alpha * Ua[0, :] + (1 - alpha) * Ub[0, :]) / (g * d) ** 0.5, Y[:] / d, ":m"
)
