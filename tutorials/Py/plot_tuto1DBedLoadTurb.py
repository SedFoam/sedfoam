...
Theta = fluidfoam.readscalar(sol, timeStep, 'Theta')
...
plt.plot(vxPDEM[I], zDEM[I]/d, 'k--', label=r'Solid phase DEM')
I = np.where(phi>0.001)[0]
plt.plot(vxPart[I], z[I]/d, label=r'Solid phase SedFoam')
plt.plot(vxFDEM, zDEM/d, 'k-.', label=r'Fluid phase DEM')
plt.plot(vxFluid, z/d, label=r'Fluid phase SedFoam')
plt.xlabel(r'$v_x^p,v_x^f$', fontsize=25)
plt.ylim([-1.525, 32.025])
plt.grid()
ax = plt.gca()
ax.set_yticklabels([])
...
plt.subplot(144)
plt.plot(Theta[I], z[I]/d, label=r'SedFoam')
