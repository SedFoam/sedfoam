...
if Nt > 1:
    ...
    plot(t_pvb + tadj, zint_pvb + 0.1, "ob", t_pvb + tadj, zint2_pvb + 0.1, "or")
    ...
    show(block=False)
    savefig("Figures/res1_tuto1.png", facecolor="w", edgecolor="w", format="png")

#    toto=raw_input('hit a key to continue')
#
# Figure 2: Concentration profiles
#
figure(num=2, figsize=(figwidth, figheight2), dpi=60, facecolor="w", edgecolor="w")
...
for i in np.arange(4):
    ...
    ax.plot(alphat[:, inum], Y[:], "-r", as_pvb[:, iexp], z_pvb[:, iexp] + 0.1, "--b")
