...
#
ax1 = subplot(gs[0, 0])
p1_5 = ax1.plot(ub, (y + Z0num) / (2.0 * rs), "-b")
p1_4 = ax1.plot(ua, (y + Z0num) / (2.0 * rs), "-r")
p1_1 = ax1.plot(Uexp[:], Zexp[:] / (2.0 * rs), "ok", markersize=ms)
ylabel(r"$y/d_p$")
xlabel(r"$U (m/s)$")
...
#
ax2 = subplot(gs[0, 1])
p2_3 = ax2.plot(alpha0, (y + Z0num) / (2.0 * rs), "-r", label=label)
...
#
ax3 = subplot(gs[0, 2])
p3_2 = ax3.plot(Tauxy, (y + Z0num) / (2.0 * rs), "-b")
p3_2 = ax3.plot(Tausxy, (y + Z0num) / (2.0 * rs), "--r")
p3_1 = ax3.plot(
    rho_f * uwexp, Zexp / (2.0 * rs), "ok", markersize=ms, label="experiment"
)
ax3.set_yticklabels([])
xlabel(r"$\tau_{xz} (Pa)$")
...
#
ax4 = subplot(gs[0, 3])
p4_2 = ax4.plot(k, (y + Z0num) / (2.0 * rs), "-b", label="TKE: k")
p4_2 = ax4.plot(
    1.5 * Theta, (y + Z0num) / (2.0 * rs), "--r", label=r"Gran. Temp.: $3/2\theta$"
)
