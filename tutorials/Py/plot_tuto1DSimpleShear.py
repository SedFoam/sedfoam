...
ax2 = subplot(gs[0, 1])
l21, = ax2.plot(Ua[0,:]/U, Y[:]/H, '-r',label='sedFoam')
...
ax3 = subplot(gs[0, 2])
l31, = ax3.semilogx(Theta[:]/(U*U), Y[:]/H, '-r')
l32, = ax3.plot(thetaexp, yexptheta, 'ob')
ax3.set_xlabel(r'$\theta/U^2$')
ax3.set_xlim(1e-4,  1e-2)
ax3.set_ylim(zmin, zmax)
ax3.set_yticklabels([''])

savefig('Figures/res_tutoSimpleShear.png', facecolor='w', edgecolor='w', format='png')

show(block=True)

phi_interp = np.interp(Y[:]/H,yexpphi, phiexp);
rms_phi = rms(phi_interp - alpha[:])
assert(rms_phi<=0.02)

u_interp = np.interp(Y[:]/H,yexp, uexp);
rms_u = rms(u_interp - Ua[0,:])
assert(rms_u<=0.03)

theta_interp = np.interp(Y[:]/H,yexptheta, thetaexp);
rms_theta = rms(theta_interp - Theta[:])/np.mean(thetaexp)
