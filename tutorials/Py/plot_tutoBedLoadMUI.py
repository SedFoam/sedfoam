...
ax2 = subplot(gs[0, 1])
l22, = ax2.plot(U[:], Y[:], '-r', label="U SEDFOAM")
l2mui, = ax2.plot(UMuI[:], yMuI[:], '--g', label="Mu(Iv) (Numerical)")
l2, = ax2.plot(uex[:], xex[:], ':k', label="Coulomb2D (Analytical)")
ax2.set_xlabel('u ($m/s$)')
ax2.set_xlim(0,  np.max(uex) * 1.1)
ax2.set_ylim(zmin, zmax)
ax2.set_yticklabels([''])
handles, labels = ax2.get_legend_handles_labels()
ax2.legend(handles, labels, loc=4, prop={'size': 16})

ax3 = subplot(gs[0, 2])
l31, = ax3.plot(pff[:], Y[:], '-r')
l3mui, = ax3.plot(PpMuI[:], yMuI[:], '--g')
...
savefig('Figures/res2_tuto2.png', facecolor='w', edgecolor='w', format='png')
